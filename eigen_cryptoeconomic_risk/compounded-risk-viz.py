...
if st.button("Update State") or True:  # Using "or True" for demonstration; in practice, this is not needed.
    np.random.seed(0)  # Reset random seed for reproducibility

    operators = [f"Operator {i+1}" for i in range(num_operators)]
    avss = []

    # Generate AVSs with attributes including a corrected entrenchment level influence
    for i in range(num_avss):
        operator = operators[i % len(operators)]
        category = np.random.randint(0, 3) if np.random.rand() >= category_dominance / 100.0 else 0
        ...
        avss.append({
            ...
            'operator': operator,
            # Entrenchment level from slider, not randomized here
    ...
    # Calculate positions and adjust them
    ...
    # Update layout
    fig.update_layout(showlegend=True, legend=dict(x=1, y=1, font=dict(size=12), itemsizing='constant'), width=1200, height=800)
