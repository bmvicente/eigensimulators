...
def main():
    ...
    with st.expander("How this Simulator Works & Basic Assumptions"):
        st.markdown("""
            ...
            The most significant parameter is the first: Cost-of-Corruption/Profit-from-Corruption relationship, since it poses the greatest weight on an AVS being exposed to corruption or to being cryptoeconomically secured. Although for the case of Shared Sequencers, it is infeasible to post invalid states and extract funds from users there the Likelihood assumed was low: "<i>The execution layer handles transaction execution, including the exclusion of invalid transactions. As a result, the profits extracted are comparatively limited when compared to a Layer 1 or a bridge, as the shared sequencer cannot post invalid states and steal money from users.</i>"
        """)
