...
###########################################################
    ...
    **AVS Reward Emission:**
    ...
    with col80:
        ...
        with col93:
            avs1_comp_loss_weight = st.slider("**Potential Compounded Loss(ΨAVS1)/ Actual Slash(δj) Ratio Factor Weight**", min_value=10, max_value=90, value=50, format='%d%%', key="avs1_clac_w")
...
""", unsafe_allow_html=True)
    ...
    with col81:
        ...
        with col93:
            avs2_comp_loss_weight = st.slider("**Potential Compounded Loss(ΨAVS2)/ Actual Slash(δj) Ratio Factor Weight**", min_value=10, max_value=90, value=50, format='%d%%', key="avs2_clac_w")
...
""", unsafe_allow_html=True)
    ...
    with col82:
        ...
        with col93:
            avs3_comp_loss_weight = st.slider("**Potential Compounded Loss(ΨAVS3)/ Actual Slash(δj) Ratio Factor Weight**", min_value=10, max_value=90, value=50, format='%d%%', key="avs3_clac_w")
...
""", unsafe_allow_html=True)
    ...
    with col97:
        ...
        if len(set(results.values())) == 1:
            recommendation = "Considering each AVS in insolation, the LRT protocol should expect the <b>same risk-adjusted return on AVS1, AVS2, and AVS3</b>."
        else:
            ...
            if len(grouped_results) == 2:  # If two groups, one has the greater, other two are equal
                ...
                if len(grouped_results[greater_value_group]) > 1:  # If the greater group has more than one AVS
                    recommendation = f"""
                        <div>
                            <p style="font-size: 18px !important;">Considering each AVS in isolation, the LRT protocol should expect:</p>
                    ...
                    recommendation = f"""
                        <div>
                            <p style="font-size: 18px;">Considering each AVS in isolation, the LRT protocol should expect:</p>
    ...
    **The Sharpe Ratio results represent the amount of net yield (after accounting for expected slashes) AVSs may earn going forward for each unit of risk, as measured by the standard deviation of those net yields over a previous time period.**
    ...
    The Sharpe Ratio's denominator, representing the standard deviation of excess returns, gauges an investment's volatility, or risk. It measures how returns deviate from their average, crucial for assessing risk-adjusted returns. Essentially, it adjusts for risk by dividing the investment's excess return by its volatility, illustrating the excess return per unit of risk. A higher standard deviation implies more risk, indicating that returns vary more from the average and require a higher excess return for the same Sharpe Ratio. This allows investors to compare investments on a risk-adjusted basis, distinguishing those that outperform due to genuine risk management rather than merely higher risk exposure.
