...
##########################################
    ...
    with st.expander("Logic"):
                st.markdown(f"""
                    ...
                    - If Stakesure insurance available > Σ Ψj and if all the AVSs bought an appropriate amount of insurance, the system is cryptoeconomically secure. If Stakesure insurance available < Σ Ψj and if not all the AVSs bought an appropriate amount of insurance, the system is cryptoeconomically insecure. Mind that it would also be cryptoeconomically insecure if Stakesure insurance avaiable > Σ Ψj and not all AVSs bought an appropriate amount of insurance.
                    - In the "Didn't Buy Insurance" all the Ψ amount for that AVS will be represented in the cryptoeconomic buffer at the bottom and on the "Bought inappropriate amount of insurance" a slider gets displyed that enables the user to set the percentage of Ψ that is uninsured, and should therefore be covered by the buffer too.
                    ...
                    - The **Buffer** takes the leftover (if positive) of the Staksure reserve that hasn't been used + if needed, the remaining half of Operator's slashed stake to adequately accomodate for uninsured or poorly insured users. (                    Half the Operator slashed amount is not accounted for the Insurance Reserve to either be burnt or allocated to a cryptoeconomic buffer to safeguard against irrational users that have not bought enough insurance or bought no insurance at all.)
                    ...
                    By offering an hindsight view, this simulation model also helps answer the question: *How much insurance should honest AVSs buy?* 
