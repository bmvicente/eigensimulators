        ...
        for token, token_results in results.items():
            ...
            for token in tokens:  # Iterate over tokens
                with st.expander(f"**Insights for {token}**"):  # Create an expander for each token
                    ...
                    # Generate insights dynamically based on the token
                    if token == "weETH":
                        token_description = """
                    ...
                    #token_results = fetch_metrics_for_token(token)  # Your function that returns metrics dynamically for the token
