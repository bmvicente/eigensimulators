    ...
    with col_results:
        ...
        for token, token_results in results.items():
            ...
            with st.expander(f"**Insights for {token}**"):
                ...
                if token == "weETH":
                    token_description = """
weETH is a wrapped, non-rebasing token representing staked eETH within the EtherFi protocol. Designed as a Liquid Restaking Token (LRT), weETH enables users to stake their ETH, earn staking rewards, and restake in EigenLayer for additional yield opportunities. EtherFi’s architecture leverages a non-custodial staking model, ensuring users retain full ownership of their staked assets while delegating validation rights to a decentralized network of node operators. This approach minimizes centralization risks and enhances trust, making weETH a robust, versatile asset for integration into DeFi ecosystems. Its utility as a high-quality collateral asset is further bolstered by its liquidity and compatibility with a wide array of protocols.
...
*More weETH details on [tokensight's LRT risk model](https://ebisu-lrt-risk-dashboard.streamlit.app/) (exclusively provided to Ebisu), [u--1's Ether.fi section](https://u--1.com/lrt/etherfi/), and [Gauntlet's weETH dashboard](https://dashboards.gauntlet.xyz/applied-research/eigenlayer/assets/0xcd5fe23c85820f7b72d0926fc9b05b43e359b7ee).*
                    ...
                    token_description = """
sUSDe, the reward-bearing token within the Ethena protocol, represents staked USDe—a delta-neutral synthetic dollar. Designed to appreciate over time through accrued protocol-generated rewards, sUSDe eliminates the need for active yield distribution, simplifying user interactions. Its architecture, based on a "Token Vault" mechanism, ensures the value of sUSDe never drops below the USDe initially staked, providing an inherent safeguard against losses. By integrating staking returns, funding basis spreads, and protocol revenue, sUSDe is positioned as a stable and yield-generating asset suitable for diverse DeFi applications.
...
*More sUSDe details on [Ethena's official dashboard](https://app.ethena.fi/dashboards/transparency) and [DefiLlama's USDe section](https://defillama.com/protocol/ethena-usde).*
                    ...
                    token_description = """
LBTC is a liquid staked Bitcoin (LST) token developed by Lombard, designed to integrate Bitcoin into the decentralized finance (DeFi) ecosystem. Each LBTC token is backed 1:1 by Bitcoin, enabling holders to earn native yields through staking while maintaining the value of their original asset. This design transforms Bitcoin from a static store of value into a dynamic financial tool capable of generating yield and participating in DeFi across multiple blockchain ecosystems. 
...
*More LBTC details on [LBTC's Dune dashboard](https://dune.com/lombard_protocol/reserves).*
            ...
            if token == "weETH":
                st.write("---")
                st.write("**Additional Ether.fi LRT (weETH) Metrics, based on Current AVS Portfolio Risk Assessment** ([model](https://ebisu-lrt-risk-dashboard.streamlit.app/)): ⚠️ ***Riskiest LRT*** ⚠️")
                
                # Add custom metrics for "weETH"
                cols = st.columns(2)
                cols[0].metric("Deposit Cap USD Share Relative to Other LRTs *(Total Pool = $100M)*", "$3.5M (Lowest Allowed)")
                cols[1].metric("MCR Relative to Other LRTs", "171% (Highest)") 
