    ...
    with col1:
        ...
        with st.expander("Logic"):
                st.markdown(f"""
                    ...
                    - The **Sufficiently-High TVL** value to assure a comfortable security level for the AVS should be at least **\${avs_total_staked:,}** (we consider this value to be double the Minimum TVL as an adequate approximation, although other risk factors should be accounted for as well).
...
#########################################
    ...
    st.write(f"""
                The **AVS Reward Emission** simulator percentage results suggest how much to distribute among $AVS Stakers, xETH Restakers and AVS Operators. To build on the *Assumptions* explanation on top, this calculation is based on the correlated reward-to-risk sensitivity per variable input -- the *Logic* in each variable help understand the rationale.
